...
- [x] **Фаза 6 — приглашения.** Токен на 7 дней, `POST /dates/{id}/send`,
      публичный `/i/:token` без входа, пять кадров сюжета, убегающая кнопка
      «Нет», share-sheet с запасным копированием в буфер.
- [x] **Фаза 7 — уведомления.** VAPID без Firebase, подписка с проверками
      порядка из ТЗ 13.2, удаление мёртвых подписок, напоминания за сутки
      и за 2 часа через APScheduler с дедупликацией флагами в `dates`.
- [x] **Фаза 8 — полировка.** Офлайн-полоска «Показываю, что помню»,
      замирание сфер в офлайне, ленивая загрузка экранов по маршрутам
      (стартовый бандл 163 КБ gzip при бюджете 200), аудит анимаций
      и вибрации.
...
6. **Lenis не используется.** ТЗ 8.3 само требует `syncTouch: false`,
   потому что родная инерция Safari лучше, и оставляет Lenis «только ради
   синхронизации с ScrollTrigger». ScrollTrigger не используется нигде,
   а десктоп заблокирован — библиотека весила бы бандл, не делая ничего
   на единственной платформе, где приложение работает.
