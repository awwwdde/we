...
## Заведение пользователей без терминала

У гостевого проекта на панели нет консоли, поэтому `python -m app.cli`
выполнить негде. Пользователи и первый инвайт-код заводятся **переменными
окружения** — на старте контейнера приложение создаёт их само.
...
Формат: `логин:Имя:цвет` и `логин:КОД` через запятую. Цвет — `ember`
или `iris`. Код придумывается свой, не короче 8 символов.

Обе операции идемпотентны: выполняются на каждом старте, но повторно
ничего не создают.

**Инвайт из окружения действует, только пока у человека нет ни одного
passkey.** После привязки ключа код перестаёт работать сам — даже если
переменная осталась в проекте. Убрать её после настройки всё равно
правильно, но дыры она не оставляет.

Дальше на телефоне: открыть сайт → «У меня код приглашения» → ввести код →
Face ID → **сохранить 10 кодов восстановления** (показываются один раз).
