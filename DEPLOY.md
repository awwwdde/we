...
```
VAPID_PUBLIC_KEY       — уведомления
VAPID_PRIVATE_KEY      — уведомления
...
Без ключей VAPID приложение работает, просто не шлёт уведомления:
эндпоинты отвечают 503, планировщик напоминаний не стартует.

Пара ключей уже сгенерирована и лежит в `.env.example` — её же можно
взять на прод. Сгенерировать новую:

```bash
python -c "from py_vapid import Vapid01; v=Vapid01(); v.generate_keys(); v.save_key('private.pem')"
```

⚠️ **Смена ключей делает все существующие подписки недействительными** —
придётся заново включать уведомления на каждом устройстве.
